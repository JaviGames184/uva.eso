 ...
 *
 * Línea: Cada una de las líneas de la entrada
 * Argumento: Se utiliza para guardar cada una de las palabras que se tienen en la entrada
 * Redirección: Se utiliza para indicar si es necesario realizar redirección (redireccion != 0) o no (redireccion == 0)
 * 				Devuelve un -1 si se encuentra más de una redirección o si no se tienen los argumentos necesarios
 ...
 *
 * Argumento: Cada uno de los argumentos que utilizará el comando
 * Narg: Número de argumentos
 * Redirección: Se utiliza para indicar si es necesario realizar redirección (redireccion != 0) o no (redireccion == 0)
 *				Contiene un -1 si se encuentra más de una redirección o si no se tienen los argumentos necesarios
 * Narg: Devuelve el número de argumentos que se tienen en la línea
 ...
 */
void UVash(char* entrada, int len, int*grabacion, char**listagrabacion, int*posGrabacion);
/*************************************************************************************************/
/*
	...
	if(argc == 1){
		...
		while(1){
			...
			if(strchr(entrada, '&') == NULL){
				...
				UVash(entrada, len, &grabacion, listagrabacion, &posGrabacion);
			} else {
...
void UVash(char* entrada, int len, int*grabacion, char**listagrabacion, int*posGrabacion){
	...
	listagrabacion[posGrabacion[0]] = strdup(entrada);
	if(strspn(entrada, " \n\t") != len){
		numeroargumentos = procesarEntrada(entrada, argumento, &redireccion);

		// Comandos
		if((strcmp("exit", argumento[0]) == 0) && grabacion[0]==0){
		...
		} else if(strcmp("IniciaGrabacion", argumento[0]) == 0){
			grabacion[0] = 1;
			posGrabacion[0] = 0;
		} else if(strcmp("FinalizaGrabacion", argumento[0]) == 0){
			grabacion[0] = 0;
		} else if(strcmp("Muestra", argumento[0]) == 0){
			for(int i = 0; i<posGrabacion[0];i++){
				printf("Comando %d: %s", i,  listagrabacion[i]);
			}
		} else if(strcmp("Recupera", argumento[0]) == 0){
			int num = atoi(argumento[1]);
			if(num < posGrabacion[0]){
				numeroargumentos = procesarEntrada(listagrabacion[num], argumento, &redireccion);
		...
		} else if(grabacion[0] == 0){
				ejecutarComando(argumento, numeroargumentos, redireccion, error_message);
		}

		if(grabacion[0] == 1){
			if(strcmp("IniciaGrabacion", argumento[0]) != 0){
				posGrabacion[0]++;
			}
		}
...
void ejecutarComandoParalelo(char* entrada, char* error_message){
	...
	for(int i=0; i<numerocomandos;i++){
		...
		if(pid == -1){
			...
			int tmp = 0;
			UVash(comando[i], lencomando[i], &tmp, listagrabacion, &tmp);
			exit(0);
		}
	}
