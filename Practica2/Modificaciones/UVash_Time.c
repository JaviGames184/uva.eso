 ...
 */
void UVash(char* entrada, int len, int* modoTime);
/*************************************************************************************************/
/*
...
void UVash(char* entrada, int len, int* modoTime){
	...
	if(strspn(entrada, " \n\t") != len){
		...
		if(strcmp("exit", argumento[0]) == 0){
		...
		} else if(strcmp("ActivaTime", argumento[0]) == 0){
			modoTime[0] = 1;
		} else if(strcmp("DesactivaTime", argumento[0]) == 0) {
			modoTime[0] = 0;
		} else if(modoTime[0] == 0){
			...
			if(pid == -1){
				...
				// Proceso hijo
				FILE* ft = fopen("salida.out", "a");
				fwrite(copiaentrada, sizeof(char), len, ft);
				fclose(ft);
...
void ejecutarComandoParalelo(char* entrada, char* error_message){
	...
	while((elemento=strsep(&entrada, "&\n")) != NULL){
		...
		// No contamos elementos vaciosficheroTime
		if(strlen(elemento) > 0){
	...
	for(int i=0; i<numerocomandos;i++){
		...
		if(pid == -1){
			...
			int tmp = 0;
			UVash(comando[i], lencomando[i], &tmp);
			exit(0);
		}
	}
